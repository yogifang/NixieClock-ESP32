...
void WiFiSettingsClass::portal() {
  ...
  http.on("/", HTTP_GET, [this, &http, &num_networks, &redirect]() {
    ...
    if (timezone.length() != 0) {
      iOldTZ = timezone.toInt() ; 
    }
   // Serial.println(strTZ) ;
      ...
      for (iCnt= -12 ; iCnt < 13 ; iCnt++ ){
        ...
        opt.replace("{code}", String(iCnt));
        opt.replace("{name}", String(iCnt));
        opt.replace("{sel}", iCnt == iOldTZ ? " selected" : "");
        http.sendContent(opt);
      }
  ...
  http.on("/", HTTP_POST, [this, &http]() {
   ...
   timezone = http.arg("timezone") ;
     if (!spurt("/timezone", timezone))
        ok = false;

    //Serial.println(strTimeZone.c_str()) ;
...
void WiFiSettingsClass::begin() {
  ...
  if (!secure) {
    secure = checkbox(F("WiFiSettings-secure"), false, _WSL_T.portal_wpa);
  }

  if (!password.length()) {
    password =
        string(F("WiFiSettings-password"), 8, 63, "", _WSL_T.portal_password);
    if (password == "") {
